...
LOG_AUDIT_PATH = Path(os.getenv("LOG_AUDIT_PATH", "data/audit.jsonl"))
...
def log_audit_event(event: str, payload: dict[str, Any] | None = None) -> None:
    """Ghi trực tiếp các sự kiện bảo mật vào file audit.jsonl"""
    import json
    from datetime import datetime
    
    LOG_AUDIT_PATH.parent.mkdir(parents=True, exist_ok=True)
    
    # Sử dụng bộ lọc PII có sẵn để bảo vệ dữ liệu trong Audit log
    safe_event = scrub_text(event)
    safe_payload = {}
    if payload:
        safe_payload = {
